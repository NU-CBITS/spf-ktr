...
app.actions.setPage = function (pageContents) {
  ...
  $("a.image").on("click",function(ev){
      ev.preventDefault();console.log(ev);app.actions.loadImage(ev.currentTarget.href, $(ev.currentTarget).data('styles'));
  });
  ...
  $("a.audio").on("click",function(ev){ev.preventDefault();console.log(ev);app.actions.loadAudio(ev.currentTarget.href);});
  $("a.audioImage").on("click",function(ev){ev.preventDefault();console.log(ev);app.actions.loadAudioImage(ev.currentTarget.href);});
  $(".definition").tooltip();
  $(".definition").on("click",function(ev){ev.preventDefault();});
  $(".skin-colors button").on("click", function(ev){ev.preventDefault();app.actions.setSkinTone(ev.currentTarget.href);})
